...
*/
commit :- findall((K, V), upd(K, V),  List), findall((K, V), mapsTo(K, V), List2), append(List, List2, Total), write(Total), 		removedups(Total, Ans), retractall(upd(K,_)), retractall(mapsTo(K,_)), addDB(Ans).
