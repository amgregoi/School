...
*)
fun lookup(k, Leaf) = raise LookupError (*changed db to Leaf *)
| 	looku(k, Update(key, value, tl)) = if k = key then value else lookup(k, tl)
...
(*  (j, k)::tl is list returned from collect. (j = key, k = value)::tl is rest of the pairs*)
...
|   collect(Update(key, value, tl)) = [(key, value)] @ delete(key, collect(tl))
